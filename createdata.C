...
void createdata(const int & _year_=2017, const string & _name_DS_="DataUL2017_all_noDuplicates_slimmed_newMuSF_add2p5", bool useRefit=false) {
    int year = _year_;
    int Year = TMath::Abs(year);
    string pre_name = "/eos/user/q/qguo/newNTuple_UL/";
    string MC__ = "Slimmed_2p5/";
    if (year==2016) {
         pre_name="/eos/cms/store/group/phys_muon/TagAndProbe/HZZ4L/";
         MC__ = "UL/MC/postVFP/";
    }
    ...
    string outputFile_Slimmed = (pre_name + whichYear[year-2016] + MC__).c_str();
    ...
    */
    ...
    TFile *newfile = new TFile(
            (outputFile_Slimmed+name_DS+"_slimmed_newMuSF_add2p5_workspace.root").c_str()
            ,"recreate");
